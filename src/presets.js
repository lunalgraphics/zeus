export const builtInPresets = {
    ...
    "Voldemort": {"baseLength":1111,"taper":76,"realisticSeed":6792,"realisticDetail":7,"realisticDisplacement":140,"realisticBranchSeed":3684,"realisticBranchChance":0.025,"realisticBranchLength":0.3,"realisticBranchAngle":0,"realisticMaxBranchDepth":3,"coreSize":8,"corePulse":10,"softness":4,"coreColor":"#ffffff","glowDepth":6,"glowRadius":4.2,"glowColor":"#80ff00","glowNoiseType":"Fractal","glowTwitchAmount":56,"glowTwitchScale":0.01,"glowTwitchOctaves":9,"glowTwitchSeed":1},
...
/** IDs that belong to output settings (not part of presets) */
const OUTPUT_SETTING_IDS = new Set(["outputWidth", "outputHeight", "edgeFade"]);

/**
 * Read all current option values from the DOM (excluding output settings).
 */
export function getPresetFromDOM() {
    const presetData = {};
    for (const inputElem of document.querySelectorAll("#options input, #options select")) {
        if (OUTPUT_SETTING_IDS.has(inputElem.id)) continue;
...
/**
 * Read ALL option values from the DOM (including output settings).
 * Used by the renderer to get the full state.
 */
export function getAllOptionsFromDOM() {
    const data = {};
    for (const inputElem of document.querySelectorAll("#options input, #options select")) {
        if (inputElem.type === "checkbox") {
            data[inputElem.id] = inputElem.checked;
        } else {
            data[inputElem.id] = inputElem.value;
            if (!isNaN(inputElem.value)) data[inputElem.id] = parseFloat(inputElem.value);
        }
    }
    return data;
}
